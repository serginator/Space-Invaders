var spaceInvaders = (function(){
    ...
    ******************************************************************/
    ...
    var canvasMinX; //The limit of the canvas for the mouse
    var canvasMinY;
    var canvasMaxX;
    ...
    ******************************************************************/
    ...
    function Star(_x, _y, _img){ //constructor for Stars
    ...
    function initStars(){ //To init the stars with random values
        circle(0,0,1);
        var littleStarImg = ctx.getImageData(0,0,1,1);
        circle(0,0,2);
        var bigStarImg = ctx.getImageData(0,0,2,2);
        var posX;
        var posY;
        var randomImg;
        for(var i = 0; i < 50; i++){
            ...
            posY = Math.floor(Math.random() * HEIGHT);
                randomImg = Math.floor(Math.random() * 2 + 1);
                if(randomImg === 1){
                        var star = new Star(posX, posY, littleStarImg);
                }
                else{
                        var star = new Star(posX, posY, bigStarImg);
                }
    ...
    function drawStars(){ //Draw stars
        for(var id in stars){
            ...
            ctx.save();
            ctx.fillStyle = "white";
			ctx.putImageData(star.imgSrc, star.x, star.y);
